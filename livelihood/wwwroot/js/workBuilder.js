...
var FieldTypes = (function () {
    ...
    $.ajax({
        'async': false,
        'global': false,
        'url': "/AppData/WorkBasicControls.json",
        'dataType': "json",
...
var AdvancedControls = (function () {
    ...
    $.ajax({
        'async': false,
        'global': false,
        'url': "/AppData/WorkAdvancedControls.json",
        'dataType': "json",
        'success': function (data) {
            json = data;
        }
...
var setup = function () {
        ...
        if (schema) {
            ...
            for (x in schema.fields) {
                ...
                if (field.type == "header") {
                    ...
                    fieldhtml += "<button type='button' class='btn btn-secondary btn-xs markup-schema-properties' data-fieldId='" + x + "' data-lock='" + field.lock + "' data-lockKey='" + field.lockKey + "' " + disable + ">Properties</button>";
                    fieldhtml += "<button type='button' class='btn btn-secondary btn-xs markup-schema-remove' data-fieldId='" + x + "' data-lock='" + field.lock + "' data-lockKey='" + field.lockKey + "' " + disable + ">Remove</button>";
                }
                    ...
                    fieldhtml += "<button type='button' class='btn btn-secondary btn-xs markup-schema-properties' data-fieldId='" + x + "' data-lock='" + field.lock + "' data-lockKey='" + field.lockKey + "'>Properties</button>";
                    if (disable == "") {
                        fieldhtml += "<button type='button' class='btn btn-secondary btn-xs markup-schema-remove' data-fieldId='" + x + "' data-lock='" + field.lock + "' data-lockKey='" + field.lockKey + "'>Remove</button>";
                    }
                }
    ...
    var positionchange = function (dropzoneIndex, draggedIndex) {
        ...
        for (x in schema.fields) {
            ...
            _schema.fields.push(schema.fields[x]);
            j++;
            if (Object.keys(schema.fields).length < parseInt(dropzoneIndex) && i == Object.keys(schema.fields).length) {
                i++;
                _schema.fields.push(draggedschema);
                j++;
            }
    ...
    var loadProperties = function (fieldId) {
        ...
        for (x in fieldData) {
            ...
            if (label == "Lock")
                label = "Field Lock";
            if (x == "required" || x == "readonly" || x == "noDuplicate" || x == "lock" || x == "visible" || x == "enableAutoNumber") {
                var checked = "";
                if (fieldData[x])
                    checked = "checked";
