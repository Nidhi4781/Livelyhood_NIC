...
(function($) {
    ...
    function addListeners(options) {
        ...
        try
        {
	        ...
	        $("#"+selected.attr('name')+"_hidden").val(selectedArray);
	        $("#"+selected.attr('name')+"_hidden").change();
        }
        catch(err)
        {
        	console.log(err);
    ...
    $.fn.sortOptions = function() {
        return this.each(function() {
        var arr = $(this).find('option').map(function(_, o) {
                return {
        ...
        var val = o2.a > o1.a ? 1 : o2.a < o1.a ? -1 : 0;
        if(val==-1){
        return val;
        }else{
